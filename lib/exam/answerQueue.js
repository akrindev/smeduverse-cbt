...
export function hasAuthError(entries) {
  return (entries ?? []).some((entry) => entry?.authError === true);
}
...
function normalizeStoredEntry(value) {
  ...
  return {
    ...
    authError: value.authError === true,
    updatedAt,
  };
}
...
export function createAnswerQueue({
  ...
  const attemptSend = async (entry) => {
    ...
    try {
      ...
      entry.authError = isAuthError(error);
      entry.updatedAt = getCurrentTimestamp(now);
    ...
    if (responseError) {
      ...
      entry.authError = isAuthError(responseError);
      entry.updatedAt = getCurrentTimestamp(now);
