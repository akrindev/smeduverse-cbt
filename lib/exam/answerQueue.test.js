...
describe("answer queue auth errors", () => {
  test("flags HTTP 401 as an auth error without retrying", async () => {
    let calls = 0;
    ...
    expect(queue.getState().entries[0].authError).toBe(true);
    expect(queue.getState().entries[0].payload.answer_chosen_id).toBe("a");
  });

  test("flags HTTP 403 as an auth error without retrying", async () => {
    let calls = 0;
    const queue = createQueue({
      send: async () => {
        calls += 1;
  ...
  test("does not flag validation errors as auth errors", async () => {
    const queue = createQueue({
      send: async () => {
    ...
    expect(queue.getState().entries[0].status).toBe("failed");
    expect(queue.getState().entries[0].authError).toBe(false);
  });

  test("persists the auth flag across reconstruction", async () => {
    ...
    expect(restoredQueue.getState().entries[0].authError).toBe(true);
    expect(restoredQueue.getState().entries[0].payload.answer_chosen_id).toBe(
      "a",
    );
  });

  test("explicit retry clears the auth flag and resends (recovery after re-login)", async () => {
    let calls = 0;
    const queue = createQueue({
      send: async () => {
        calls += 1;
...
describe("answer queue auth logout signal", () => {
  test("signals forced logout when any entry has an auth error", async () => {
    const { hasAuthError } = await import("./answerQueue");
    const queue = createQueue({
      send: async () => {
        const error = new Error("Unauthenticated.");
        error.response = { status: 401, data: { message: "Unauthenticated." } };
    ...
    expect(hasAuthError(queue.getState().entries)).toBe(false);
    ...
    expect(hasAuthError(queue.getState().entries)).toBe(true);
  });

  test("does not signal logout for validation or network failures", async () => {
    const { hasAuthError } = await import("./answerQueue");
    ...
    expect(hasAuthError(validationQueue.getState().entries)).toBe(false);

    const offlineQueue = createQueue({
      send: async () => {
    ...
    expect(hasAuthError(offlineQueue.getState().entries)).toBe(false);
  });
});
...
describe("answer queue", () => {
  ...
  test("background recovery via flush() never resets failed entries; only explicit retry() does", async () => {
    let calls = 0;
    const queue = createQueue({
      send: async () => {
        calls += 1;
        ...
        error.response = {
          status: 422,
          data: { message: "Payload tidak valid" },
        };
        throw error;
      },
    });
    ...
    expect(calls).toBe(1);
    expect(queue.getState().entries[0].status).toBe("failed");
    const attemptsAfterFailure = queue.getState().entries[0].attempts;

    // Mount, `online` event, and 10s interval ticks all run flush(): none of
    // them may resend or reset a terminal validation failure.
    await queue.flush();
    await queue.flush();

    expect(calls).toBe(1);
    expect(queue.getState().entries[0].status).toBe("failed");
    expect(queue.getState().entries[0].attempts).toBe(attemptsAfterFailure);
    expect(queue.getState().entries[0].lastError).toContain(
      "Payload tidak valid"
    );
    // The failed entry still blocks submission and surfaces the manual retry UI.
    expect(queue.getState().pendingCount).toBe(1);
    expect(canSubmitExam(queue.getState().pendingCount)).toBe(false);

    // The manual retry button / sync-required / submit path resets one cycle.
    await queue.retry();
