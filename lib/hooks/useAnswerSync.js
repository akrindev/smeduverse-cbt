...
import { createAnswerQueue, hasAuthError } from "../exam/answerQueue";
...
export function AnswerSyncProvider({ children }) {
  ...
  const authLogoutDone = useRef(false);

  useEffect(() => {
    if (authLogoutDone.current || !hasAuthError(queueState.entries)) {
      return;
    }

    authLogoutDone.current = true;
    logout();
  }, [logout, queueState.entries]);
